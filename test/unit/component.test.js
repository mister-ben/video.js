...
QUnit.test('should allow for children that are elements', function(assert) {
  ...
  const comp = new Component(getFakePlayer());
  const element = Dom.createEl('div');

  // Add element as video el gets added to player
  comp.el().appendChild(element);
  comp.children_.unshift(element);
  ...
  assert.ok(child2.el_.previousSibling === element, 'addChild should insert el before a sibling that is an element');
